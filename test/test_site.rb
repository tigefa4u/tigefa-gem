...
class TestSite < Test::Unit::TestCase
  context "configuring sites" do
    should "have an array for plugins by default" do
      site = Site.new(Tigefa::Configuration::DEFAULTS)
      assert_equal [File.join(Dir.pwd, '_plugins')], site.plugins
    end

    should "look for plugins under the site directory by default" do
      site = Site.new(Tigefa::Configuration::DEFAULTS.merge({'source' => File.expand_path(source_dir)}))
      assert_equal [File.join(source_dir, '_plugins')], site.plugins
    end

    should "have an array for plugins if passed as a string" do
      site = Site.new(Tigefa::Configuration::DEFAULTS.merge({'plugins' => '/tmp/plugins'}))
      assert_equal ['/tmp/plugins'], site.plugins
    end

    should "have an array for plugins if passed as an array" do
      site = Site.new(Tigefa::Configuration::DEFAULTS.merge({'plugins' => ['/tmp/plugins', '/tmp/otherplugins']}))
      assert_equal ['/tmp/plugins', '/tmp/otherplugins'], site.plugins
    end

    should "have an empty array for plugins if nothing is passed" do
      site = Site.new(Tigefa::Configuration::DEFAULTS.merge({'plugins' => []}))
      assert_equal [], site.plugins
    end

    should "have an empty array for plugins if nil is passed" do
      site = Site.new(Tigefa::Configuration::DEFAULTS.merge({'plugins' => nil}))
      assert_equal [], site.plugins
    end

    should "expose default baseurl" do
      site = Site.new(Tigefa::Configuration::DEFAULTS)
      assert_equal Tigefa::Configuration::DEFAULTS['baseurl'], site.baseurl
    end

    should "expose baseurl passed in from config" do
      site = Site.new(Tigefa::Configuration::DEFAULTS.merge({'baseurl' => '/blog'}))
  ...
  context "creating sites" do
    ...
    context 'error handling' do
      should "raise if destination is included in source" do
        stub(Tigefa).configuration do
          Tigefa::Configuration::DEFAULTS.merge({'source' => source_dir, 'destination' => source_dir})
        end
      ...
      should "raise if destination is source" do
        stub(Tigefa).configuration do
          Tigefa::Configuration::DEFAULTS.merge({'source' => source_dir, 'destination' => File.join(source_dir, "..")})
        end
    ...
    context 'with orphaned files in destination' do
      ...
      should 'remove orphaned files in destination - keep_files .svn' do
        config = Tigefa::Configuration::DEFAULTS.merge({'source' => source_dir, 'destination' => dest_dir, 'keep_files' => ['.svn']})
    ...
    context 'with an invalid markdown processor in the configuration' do
      should 'not throw an error at initialization time' do
        ...
        assert_nothing_raised do
          Site.new(Tigefa.configuration.merge({ 'markdown' => bad_processor }))
        end
      end

      should 'throw FatalException at process time' do
        bad_processor = 'not a processor name'
        s = Site.new(Tigefa.configuration.merge({ 'markdown' => bad_processor }))
        assert_raise Tigefa::FatalException do
          s.process
        end
      end

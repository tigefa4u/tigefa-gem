...
class TestRelatedPosts < Test::Unit::TestCase
  context "building related posts without lsi" do
    setup do
      stub(Tigefa).configuration do
        Tigefa::Configuration::DEFAULTS.merge({'source' => source_dir,
    ...
    should "use the most recent posts for related posts" do
      ...
      related_posts = Tigefa::RelatedPosts.new(last_post).build
  ...
  context "building related posts with lsi" do
    setup do
      ...
      any_instance_of(Tigefa::RelatedPosts) { |i| stub(i).display }
      @site = Site.new(Tigefa.configuration)
    end
    ...
    should "use lsi for the related posts" do
      ...
      assert_equal @site.posts[-1..-9], Tigefa::RelatedPosts.new(@site.posts.last).build
    end
  end
end
